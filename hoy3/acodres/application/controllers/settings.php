...
class Settings extends MY_Controller
{
	...
	function barrios()
	{
		$this->view_data['items'] = Barrio::find('all');
		$this->content_view = 'settings/barrios';
	}	
	function add_barrio(){
		 
			$this->theme_view = 'modal';
			$this->view_data['municipios'] = Municipio::find('all');
			$this->view_data['title'] = 'Agregar Barrio';
			$this->view_data['form_action'] = 'settings/save_barrio';
			$this->content_view = 'settings/_barrio';
	 
	}
	
function save_barrio(){
		if($_POST){
		...
		redirect('settings/barrios');
		
		
	}

 function editar_barrio($id = FALSE){
		 
			$this->view_data['items'] = Barrio::find($id);
			$this->view_data['municipios'] = Municipio::find('all');
			$this->theme_view = 'modal';
			$this->view_data['title'] = 'Editar Barrio';
			$this->view_data['form_action'] = 'settings/update_barrio';
			$this->content_view = 'settings/_edit_barrio';
	...
	function delete_barrio ($id = FALSE){
		$item = Barrio::find($id);
		$item->delete();
	...
	function tinmuebles()
	{
		$this->view_data['items'] = Tipoinmueble::find('all');
		$this->content_view = 'settings/inmuebles';
	}	
	function add_tinmuebles(){
		 
			$this->theme_view = 'modal';
 			$this->view_data['title'] = 'Agregar Tipo de Producto';
			$this->view_data['form_action'] = 'settings/save_inmuebles';
			$this->content_view = 'settings/_inmueble';
	 
	}
	
function save_inmuebles(){
		if($_POST){
			...
			unset($_POST['files']);
 			...
 			$_POST = array_map('htmlspecialchars', $_POST);
			$item = Tipoinmueble::create($_POST);
       		if(!$item){$this->session->set_flashdata('message', 'error: Problemas Al crear el registro');}
       		else{$this->session->set_flashdata('message', 'success: Registro Creado Con Exito' );}
 		}
		redirect('settings/tinmuebles');
		
		
	}

 function editar_tinmuebles($id = FALSE){
		 
			$this->view_data['items'] = Tipoinmueble::find($id);
 			$this->theme_view = 'modal';
			$this->view_data['title'] = 'Editar Tipo de Producto';
			$this->view_data['form_action'] = 'settings/update_inmuebles';
			$this->content_view = 'settings/_edit_inmuebles';
	 
	}
	
	
	function update_inmuebles( ){
		if($_POST){
			unset($_POST['send']);
			unset($_POST['files']);
			$_POST = array_map('htmlspecialchars', $_POST);
			$id = $_POST['id'];
			$item = Tipoinmueble::find($id);
			$item = $item->update_attributes($_POST);
       		if(!$item){$this->session->set_flashdata('message', 'error: Problemas al Editar el Registro' );}
       		else{$this->session->set_flashdata('message', 'success: Registro editado con exito' );}
	...
	function delete_tinmuebles ($id = FALSE){
		$item = Tipoinmueble::find($id);
		$item->delete();
		if(!$item){$this->session->set_flashdata('message', 'error: Problemas al Eliminar' );}
       	else{$this->session->set_flashdata('message', 'success:Eliminado con exito' );}
	...
	{
		$this->view_data['items'] = Operacione::find('all');
		$this->content_view = 'settings/operaciones';
	}	
	function add_operaciones(){
		 
			$this->theme_view = 'modal';
 			$this->view_data['title'] = 'Agregar Operacion';
			$this->view_data['form_action'] = 'settings/save_operaciones';
			$this->content_view = 'settings/_operaciones';
	 
	}
	
function save_operaciones(){
		if($_POST){
			...
			unset($_POST['files']);
 			...
 			$_POST = array_map('htmlspecialchars', $_POST);
			$item = Operacione::create($_POST);
       		if(!$item){$this->session->set_flashdata('message', 'error: Problemas Al crear el registro');}
       		else{$this->session->set_flashdata('message', 'success: Registro Creado Con Exito' );}
 		}
		redirect('settings/operaciones');
		
		
	}

 function editar_operaciones($id = FALSE){
		 
			$this->view_data['items'] = Operacione::find($id);
 			$this->theme_view = 'modal';
			$this->view_data['title'] = 'Editar Operacion';
			$this->view_data['form_action'] = 'settings/update_operaciones';
			$this->content_view = 'settings/_edit_operaciones';
 ...
 function delete_operaciones ($id = FALSE){
		$item = Operacione::find($id);
		$item->delete();
		if(!$item){$this->session->set_flashdata('message', 'error: Problemas al Eliminar' );}
       	else{$this->session->set_flashdata('message', 'success:Eliminado con exito' );}
 ...
 function municipios()
	{
		$this->view_data['items'] = Municipio::find('all');
		$this->content_view = 'settings/municipios';
	}
function add_municipio(){
		 
			$this->theme_view = 'modal';
			$this->view_data['dptos'] = Dpto::find('all');
			$this->view_data['title'] = 'Agregar Municipio';
			$this->view_data['form_action'] = 'settings/save_municipio';
			$this->content_view = 'settings/_municipio';
	 
	}
	
function save_municipio(){
		if($_POST){
			...
			unset($_POST['files']);
 			...
 			$_POST = array_map('htmlspecialchars', $_POST);
			$item = Municipio::create($_POST);
       		if(!$item){$this->session->set_flashdata('message', 'error: Problemas Al crear el registro');}
       		else{$this->session->set_flashdata('message', 'success: Registro Creado Con Exito' );}
 		}
		redirect('settings/municipios');
		
		
	}
 function editar_municipio($id = FALSE){
		 
			$this->view_data['items'] = Municipio::find($id);
			$this->view_data['dptos'] = Dpto::find('all');
			$this->theme_view = 'modal';
			$this->view_data['title'] = 'Editar municipio';
			$this->view_data['form_action'] = 'settings/update_municipio';
			$this->content_view = 'settings/_edit_municipio';
	...
	function delete_municipio ($id = FALSE){
		$item = Municipio::find($id);
		$item->delete();
		if(!$item){$this->session->set_flashdata('message', 'error: Problemas al Eliminar' );}
       	else{$this->session->set_flashdata('message', 'success:Eliminado con exito' );}
	...
	function user_delete($user = FALSE)
	{
		...
		$user->status = 'deleted';
		$user->save();
		$this->session->set_flashdata('message', 'Regristo Eliminado con Exito' );
		}else{
		$this->session->set_flashdata('message', 'Problema al eliminar el registro' );
		}
	...
	function user_create()
	{
		...
		}else
		{
			...
			$this->view_data['modules'] = Module::find('all', array('order' => 'orden asc', 'conditions' => array('type != ?', 'client')));
...
$this->view_data['empresas'] = Empresa::find('all');
		...
		$this->session->set_flashdata('message', ' Registro Editado Con Exito' );
 		redirect('settings/users');
 		}else{
