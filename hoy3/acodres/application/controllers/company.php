		...
		function update_ev($id){
 		...
 		$value = Empresa::find($id);
  		...
  		if($_POST){
            ...
            if(!$value){$this->session->set_flashdata('message', ' Problemas al Crear el Registro' );}
       	    else{$this->session->set_flashdata('message', ' Registro Creado con exito' );}
	...
	function save(){
		if($_POST){
			...
			$item = Quinessomo::create($_POST);
       		if(!$item){$this->session->set_flashdata('message', 'Registro Creado Con Exito');}
       		else{$this->session->set_flashdata('message', ' Registro Creado Con Exito' );}
	...
	function update( ){
		if($_POST){
			...
			$item = $item->update_attributes($_POST);
       		...
       		else{$this->session->set_flashdata('message', ' Registro editado con exito' );}
 		...
 		$value = Empresa::find($id);
 		    try {
			    ...
			    $this->session->set_flashdata('message', ' Registro eliminado con exito' );
                    ...
                    $this->session->set_flashdata('message', ' Problemas al eliminar , Violacion de Integridad ' );  
             }
		     redirect('company/mostrar');
